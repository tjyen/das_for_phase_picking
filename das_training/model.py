...
def build_model(in_channels=1, out_channels=3, reg=0.0, *args, **kwargs) -> PhaseNetDAS:
    ...
    classifier = UNetHead(in_channels=16, out_channels=out_channels, kernel_size=(7, 7), padding=(3, 3), reg=reg) #change the original model

    return PhaseNetDAS(backbone, classifier)
        
def load_model(model_name, backbone, phases, device, checkpoint_path='PhaseNet-DAS-v1.pth'):
    # 加载模型检查点
...
def load_model_new(model_name, backbone, phases, device, checkpoint_path='PhaseNet-DAS-v1.pth'):
    # 加载模型检查点
    check_point = torch.load(checkpoint_path)
