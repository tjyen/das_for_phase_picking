...
def generate_label(
    ...
    Args:
        data (tensor): [nch, nt, nx]
        phase_list (list): [[p_channel, p_index, p_direction], [s_channel, s_index, s_direction], [other phases]]
...
def generate_label_new(
    ...
    """
    nch, nt, nx = data.shape
    target = np.zeros([5, nt, nx], dtype=np.float32)  # 修改維度為 5, 分別代表(1 - p_u - p_d - s_u - s_d, p_u, p_d, s_u, s_d)
    time_mask = np.zeros([1, nt, nx], dtype=np.float32)
    ...
    for i, (picks, w) in enumerate(zip(phase_list, label_width)):
        for trace, phase_time, direction in picks:  
            trace = int(trace)
            t = np.arange(nt) - phase_time
            gaussian = np.exp(-(t**2) / (2 * (w / 6) ** 2))
            gaussian[gaussian < 0.1] = 0.0
            if direction == 1:   
                if i == 0:
                    target[1, :, trace] += gaussian  # p_u
                elif i == 1:
                    target[3, :, trace] += gaussian  # s_u
            elif direction == 2:
                if i == 0:
                    target[2, :, trace] += gaussian  # p_d
                elif i == 1:
                    target[4, :, trace] += gaussian  # s_d
            time_mask[0, int(phase_time) - w : int(phase_time) + w, trace] = 1
...
def calc_snr_new(data: torch.Tensor, picks: list, noise_window: int = 200, signal_window: int = 200):
    SNR = []
    S = []
    N = []
    for trace, phase_time, direction in picks:
...
class DASIterableDataset(IterableDataset):
    ...
    def sample_training(self, file_list):
        while True:
            ...
            for label_file in tqdm(file_list):
                ...
                try: #self.data_path + "/" + 
                    with fsspec.open(data_file, "rb") as f:
...
class DASIterableDataset_new(IterableDataset):
    def __init__(
        self,
        data_path="./",
        data_list=None,
    ...
    def sample_training(self, file_list):
        while True:
            ...
            for label_file in file_list:
                ...
                for pick_type in self.phases:
                    meta[pick_type] = picks[picks["phase_type"] == pick_type][
                        ["channel_index", "phase_index", "direction"] ## 加入direction
                    ].to_numpy()
                ...
                ## snr
                if "P" in meta:
                    snr, S, N = calc_snr_new(data, meta["P"])
                else:
                    snr, S, N = 0, 0, 0
                ...
                ## generate training labels
                targets, phase_time_mask = generate_label_new(data, picks, return_time_mask=True)
                targets = torch.from_numpy(targets)
                phase_time_mask = torch.from_numpy(phase_time_mask)
